...
export function authMode() {
    if (!TOKEN)
        return "OPEN — MCP_ACCESS_TOKEN is not set; anyone with the URL can use this server";
    if (ALLOW_OPEN)
        return "token accepted; requests without one still allowed (MCP_ALLOW_UNAUTHENTICATED) — remove once clients are updated";
    return "token required";
}
/** Is this an MCP URL ("/mcp" or "/mcp/<token>", with or without a query)? */
export function parseMcpPath(url, base = "/mcp") {
    const path = String(url ?? "").split("?")[0];
    if (path === base || path === `${base}/`)
        return { isMcp: true, pathToken: null };
    if (path.startsWith(`${base}/`)) {
        const rest = path.slice(base.length + 1);
        if (rest && !rest.includes("/")) {
            try {
                return { isMcp: true, pathToken: decodeURIComponent(rest) };
            }
            catch {
                return { isMcp: true, pathToken: rest };
            }
        }
    }
...
export function checkMcpAuth(req, pathToken) {
    if (!TOKEN)
        return { ok: true };
    ...
    if (supplied && same(supplied, TOKEN))
        return { ok: true };
    if (!supplied && ALLOW_OPEN)
        return { ok: true };
